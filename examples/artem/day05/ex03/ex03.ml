...
module Make : MAKE =
  functor (FractionnalBits : FRACTIONNAL_BITS) ->
    struct
      type t = int
      let of_float v =
        let scaled = v *. float_of_int (1 lsl FractionnalBits.bits) in
        int_of_float (if scaled >= 0.0 then scaled +. 0.5 else scaled -. 0.5)
      let of_int v = v lsl FractionnalBits.bits
      let to_float v = float_of_int v /. float_of_int (1 lsl FractionnalBits.bits)
      let to_int v = v asr FractionnalBits.bits
      ...
      let add x y = x + y
      let sub x y = x - y
      let mul x y = (x * y) asr FractionnalBits.bits
      let div x y = (x lsl FractionnalBits.bits) / y
      let foreach x y f =
        let rec loop i =
          if i <= y then begin
            f i;
...
let () =
    ...
    Fixed4.foreach Fixed4.zero half (fun f -> Printf.printf "%s " (Fixed4.to_string f));
    print_endline "";
